...
function test2()
    wv = 1.0e15
    ang = collect(linspace(0.0,pi*0.5,1000))
    ml = [Layer(Cst()) ; Layer(Cst(5.0+im*3.0))]
    eps1 = permittivity(ml[1].material,wv)
    kx = compute_kx.(ang,[eps1],[wv])
    RTE=zeros(Float64,1000)
    RTM=zeros(Float64,1000)

    for i=1:1000
        (rte,tte) = rt(ml, te(), kx[i] ,wv)
        (rtm,ttm) = rt(ml, tm(), kx[i] ,wv)
         RTE[i] = abs(rte)^2
         RTM[i] = abs(rtm)^2
    end
    plot(ang./pi.*180,RTE,xlim=(0,90),ylim=(0,1))
...
function test3()
    wv = 1.0e15
    ang = collect(linspace(0.0,pi*0.5,1000))
    ml = [Layer(Cst(5.0+im*3.0)) ; Layer(Cst(10.0+im*3.0))]
    eps1 = permittivity(ml[1].material,wv)
    kx = compute_kx.(ang,[eps1],[wv])
    RTE=zeros(Float64,1000)
    RTM=zeros(Float64,1000)
...
function test4()
    wv = 1.0e13
    ang = collect(linspace(0.0,pi*0.5,1000))
    ml = [Layer(Cst()) ; Layer(Cst(5.0+im*3.0),1.0e-6) ; Layer(Cst(10.0+im*3.0))]
    eps1 = permittivity(ml[1].material,wv)
    kx = compute_kx.(ang,[eps1],[wv])
    RTE=zeros(Float64,1000)
    RTM=zeros(Float64,1000)
...
function test5()
    ...
    th = 1.0e-6
    ang = collect(linspace(0.0,pi*0.5,1000))
    ml = [Layer(Cst()) ; Layer(Cst(5.0+im*3.0),th) ; Layer(Cst(10.0+im*3.0))]
    eps1 = permittivity(ml[1].material,wv)
    kx = compute_kx.(ang,[eps1],[wv])
    RTE=zeros(Float64,1000)
    RTM=zeros(Float64,1000)
    ...
    for i=1:1000
        (rte01,tte) = rt(Bulk(Cst(),Cst(5.0+im*3.0)), te(), kx[i] ,wv)
        (rtm01,ttm) = rt(Bulk(Cst(),Cst(5.0+im*3.0)), tm(), kx[i] ,wv)

        (rte12,tte) = rt(Bulk(Cst(5.0+im*3.0),Cst(10.0+im*3.0)), te(), kx[i] ,wv)
        (rtm12,ttm) = rt(Bulk(Cst(5.0+im*3.0),Cst(10.0+im*3.0)), tm(), kx[i] ,wv)

        kz = compute_kz(kx[i],5.0+im*3.0,wv)
...
function test6(ang)
    ...
    for i=1:1000
        (rte,tte) = rt(ml, te(), kx[i] ,wv[i])
        (rtm,ttm) = rt(ml, tm(), kx[i] ,wv[i])
         RTE[i] = abs(rte)^2
         RTM[i] = abs(rtm)^2
    end
...
function test7(T,th)
    ...
    wv = collect(linspace(1.0e13,1.0e15,1000))
    ml = [Layer(Cst()) ; Layer(Si(),th) ; Layer(Al())]
    em = zeros(Float64,1000)
    for i=1:1000
       em[i] = emissivity_w(ml, wv[i])
    end
...
function test8(T1,T2,dist,dim)
    ...
    for i=1:dim
       qw_ev_te[i] = heat_transfer_w(Evanescent() ,ml, ml, gapl ,te() ,wv[i], T1,T2)
       qw_ev_tm[i] = heat_transfer_w(Evanescent() ,ml, ml, gapl ,tm() ,wv[i], T1,T2)
       qw_prop_te[i] = heat_transfer_w(Propagative() ,ml, ml, gapl ,te() ,wv[i], T1,T2)
       qw_prop_tm[i] = heat_transfer_w(Propagative() ,ml, ml, gapl ,tm() ,wv[i], T1,T2)
    end
 ...
 function test9(dist)
     wv = 1.0e15
     kx = collect(linspace(wv/c0+1.0e-10,8.0*wv/c0,1000))
     ml = [Layer(Al())]
     gapl  = Layer(Cst(),dist)
     tkx_w = zeros(Float64,1000)
     for i=1:1000
         tkx_w[i] = kx[i]*transmission_kx_w(Evanescent() ,ml,ml,gapl,tm(),kx[i],wv)
         println(tkx_w[i])
    end
    plot(kx,tkx_w)
    xaxis!(:log10)
  ...
  function test10(T1,T2,dim)
      ...
      for i=1:dim
          ...
          gapl   = Layer(Cst(),dist[i])
          q[i,:] = total_heat_transfer(ml, ml, gapl, T1,T2)
...
" compute kx-omega map of emissivity"
    function test12(th,dimw,dimk)
      ml = [Layer(Cst()) ; Layer(Sic(),th) ; Layer(Al())]
      wv = collect(linspace(1.0e13,1.0e15,dimw))
      em = zeros(Float64,dimw*dimk,3)
      count = 1
      for i = 1:dimw
        kx = collect(linspace(1.0e4,wv[i]/c0,dimk))
        for j = 1:dimk
            em[count,1] = kx[j]
            em[count,2] = wv[i]
            em[count,3] = emissivity_kx_w(ml, kx[j], wv[i])
            count += 1
        end
      end
...
" compute dispersion relation SPP for Air-SiC interface"
    function test13(dim)
          wv = collect(linspace(1.0e13,1.0e15,dim))
          eps1 = permittivity.([Cst()],wv)
          eps2 = permittivity.([Sic()],wv)
          kpar = kssp.(eps1,eps2,wv)
          kl   = real(sqrt.(eps2)).*wv/c0
    ...
    function test14(dim)
          wv = collect(linspace(1.0e13,1.0e15,dim))
          eps1 = permittivity.([Sic()],wv)
          eps2 = permittivity.([Al()],wv)
          kpar = kssp.(eps1,eps2,wv)
          kl   = real(sqrt.(eps2)).*wv/c0
          output_data = [real(kpar) imag(kpar) wv kl]
...
" compute total emissivity and compare with classical formula for far-field transfer"
    function test16(th,T1,T2)
      ml = [Layer(Cst()) ; Layer(Sic(),th) ; Layer(Al())]
      em_ml = emissivity(ml, T1)

      b = Bulk(Cst(),Al())
      em_b = emissivity(b, T2)
      hf = farfield_transfer(em_ml , em_b, T1, T2)
 ...
 " compute total emissivity for two bulk media and compare with classical formula for far-field transfer"
        function test17(T1,T2)
          b1 = Bulk(Cst(),Sic())
          em_ml = emissivity(b1, T1)

          b2 = Bulk(Cst(),Al())
          em_b = emissivity(b2, T2)
          hf = farfield_transfer(em_ml , em_b, T1, T2)
